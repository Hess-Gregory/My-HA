const MAP_STYLES = {
  kmi_light: {
    name: 'KMI Light',
    url: 'https://tile.meteo.be/styles/Light-v10-nl/{z}/{x}/{y}.png',
    attribution: '© OpenStreetMap contributors | Radar data: www.meteo.be | Leaflet',
    maxZoom: 12,
  },
  osm_standard: {
  ...
  osm_humanitarian: {
    ...
    attribution: '© OpenStreetMap contributors, Humanitarian OpenStreetMap Team | Radar data: www.meteo.be | Leaflet',
    maxZoom: 19,
  },
  cartodb_positron: {
    name: 'CartoDB Positron',
    url: 'https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png',
    attribution: '© OpenStreetMap contributors | © CARTO | Radar data: www.meteo.be | Leaflet',
    maxZoom: 20,
  },
  cartodb_voyager: {
  ...
  esri_gray: {
    ...
    attribution: 'Tiles © Esri | Radar data: www.meteo.be | Leaflet',
    maxZoom: 16,
  },
};
...
class KmiWeatherRadarBetaCard extends HTMLElement {
  ...
  getMapStyle() {
    const key = this.config.map_style || 'kmi_light';
    return MAP_STYLES[key] || MAP_STYLES.kmi_light;
  }

  getTileLayerConfig() {
    const style = this.getMapStyle();
    return {
      url: this.config.tile_url || style.url,
      attribution: this.config.tile_url
        ? (this.config.custom_attribution || 'Custom map layer | Radar data: www.meteo.be | Leaflet')
        : style.attribution,
      maxZoom: Number(this.config.tile_max_zoom || style.maxZoom || 19),
    };
  }
  ...
  async start() {
    try {
      ...
      const tileLayerConfig = this.getTileLayerConfig();
      this._tileLayer = L.tileLayer(tileLayerConfig.url, {
  ...
  async loadFrames(forceLatest = false) {
    ...
    const validFrames = new Set(newFrames);
    for (const cachedFile of this._frameCache.keys()) {
      if (!validFrames.has(cachedFile)) this._frameCache.delete(cachedFile);
    }
  ...
  async showFrame(newIdx, retryAfterRefresh = true) {
    ...
    try {
      ...
      let geojson = this._frameCache.get(file);
      if (!geojson) {
        const res = await this.fetchApi(`/api/kmi_weather_radar_beta/frame/${encodeURIComponent(file)}`);
        if (!res.ok) {
        ...
        geojson = geobuf.decode(new Pbf(new Uint8Array(buffer)));
        geojson.features.sort((a, b) => Number(a.properties.value || 0) - Number(b.properties.value || 0));
        this._frameCache.set(file, geojson);
      }
